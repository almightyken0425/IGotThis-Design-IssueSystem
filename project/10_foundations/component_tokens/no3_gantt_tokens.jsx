// ─────────────────────────────────────────────────────────────
// GANTT_TOKENS · 甘特圖與導航組專屬元件參數
//
// 消費端：20_components/no3_gantt_nav.jsx 的 GanttHeader / GanttTimeline /
// GanttBar / SortableRow / SectionDivider / LevelSwitcher / Toolbar，
// 最終服務 DevOrderScreen（開發順序表）。
//
// 兩類值分開放：
...
//   假日多為連續兩格，成塊後辨識足夠，單格也還看得見，且淡到不與長條爭視線。
// dark 疊暖白 sand[0]，把 surface sand[900] 抬成 #312D24，對 surface 亮度比 1.17。
//   alpha 卡在 0.055 是因為再抬就吃掉整片 timeline 的前景預算。
//   暗主題抬得比亮主題壓得少，屬對比預算而非手感。
//
// 假日格上的文字只出現在 header 的日號一處，那裡的底是「header dim + 假日 tint」
// 雙層而非單層 tint。舊註解引的 4.89 / 4.73 是量單層（timeline 本體）得來的，
// 但 timeline 本體並無文字；量雙層後 text.tertiary 只剩 4.10–4.54，落在 AA 之下。
// 故日號改吃 text.secondary（同底 5.97–9.19），詳見 no3_gantt_nav.jsx 的 GanttHeader。
const GANTT_HOLIDAY_TINT_ALPHA = {
...
function resolveGanttColors(theme) {
  ...
  return {
    ...
    holidayCell:     GANTT_TOKENS.HOLIDAY_CELL_BG[theme.id] || GANTT_TOKENS.HOLIDAY_CELL_BG.light,
...
Object.assign(window, { GANTT_TOKENS, resolveGanttColors });
