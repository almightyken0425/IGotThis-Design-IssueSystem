// ─────────────────────────────────────────────────────────────
// DATA_DISPLAY_TOKENS · 資料展示元件參數（表格 / 看板 / 空狀態 / 過濾標示）
//
// 消費端：20_components/no2_data_display.jsx 的 DataTable、TableCell 系列、
// KanbanColumn、KanbanCard、EmptyState、FilterNotice。
// 畫面端對應 spec 的 ListScreen 工單表格與 KanbanScreen 看板欄。
//
// 兩段結構：
//   DATA_DISPLAY_TOKENS — 幾何與字體，全數引用 atomic 層階梯。
//   DATA_DISPLAY_COLORS — 色彩不存值、存「吃 theme 的哪個鍵」，每組是一個
//                          吃 theme 物件的 resolver。同一份 token 因此在
//                          THEME_LIGHT 與 THEME_DARK 兩邊都成立、元件不寫死主題。
//
// 禁止在本檔硬寫 hex 或 raw number；視覺校準例外一律標 // (literal: <原因>)。
// ─────────────────────────────────────────────────────────────

const DATA_DISPLAY_TOKENS = {
  // canvas 自製 glyph 的筆畫寬。本組 glyph 走 16 格線稿（控件組走 24 格），
  // 同一個視覺粗細換算過來比控件組細一階。消費端只讀本鍵。
  GLYPH_STROKE:                1.5,                        // (literal: 16 格線稿的筆畫粗細，視覺校準)

  // ─── 工單表格 ───────────────────────────────────────────────
  TABLE: {
    // row 高度三檔直接吃 ROW_HEIGHT；density prop 用同一組鍵名（compact / base / relaxed）。
    ...
    CELL_PADDING_X_BY_DENSITY: { compact: SPACING.sm, base: SPACING.md, relaxed: SPACING.md },
    ...
    // 格內元件。badge 與 avatar 交給基礎元件組的 Badge / Avatar 渲染，
    // 這裡只留「用哪一檔」與 fallback 幾何；數值鏡射 BADGE_TOKENS 與 AVATAR_TOKENS，
    // 兩者未載入時 fallback 的視覺仍與正式元件一致。
  ...
  KANBAN: {
    ...
    // 卡片自己的 focus 幾何。原先借 TABLE 與 EMPTY_STATE 的同名鍵，
    // 那兩組任一調整都會連坐看板卡；拆出來讓三組各自可調。
...
// ─────────────────────────────────────────────────────────────
// 色彩 resolver。每個 key 吃 theme 物件、回傳該元件用得到的色彩集合。
// 一律讀 theme 的語意鍵，不讀 PALETTE 原料、不寫 hex。
//
// 對比守則（依 atomic 層註解已驗證的配對）：
...
const DATA_DISPLAY_COLORS = {
  ...
  EMPTY_STATE: (theme) => ({
    ...
    // hover 走「同色系推一階」，方向依主題相反——與 BUTTON_TOKENS.COLORS_BY_VARIANT
    // 的 primary 同一條規則。原本兩主題都推 700 是 bug：dark 的 main 是亮色 pine[400]、
    // 文字是 on_main 深墨，推到 pine[700] 會變成深墨字壓在深底上、對比塌到 2:1 以下。
...
Object.assign(window, { DATA_DISPLAY_TOKENS, DATA_DISPLAY_COLORS });
